...
def process_df(quant, dirname, args=None, args_model=None, save=True):
    ...
    if quant.columns.names != ["stat", "layer", "width"]:
        # the order is
        # perform pivot
        quant = pd.melt(quant.reset_index(), id_vars="steps").pivot(index="steps", columns=["stat", "layer", "width"], values="value")

    idx = pd.IndexSlice
    cols_error = idx['error', :, :]
    quant.loc[:, cols_error] *= 100  # in %
    N_L = len(quant.columns.levels[1])  # number of hidden layers
    errors = quant["error"]
    losses = quant["loss"]
    ...
    bp.axes[0,0].plot(np.linspace(0, N_L, num=N_L)-1, (N_L)*[errors[0].iloc[0].values], color="red")
    ...
    bp.axes[0,1].plot(np.linspace(0, N_L, num=N_L)-1, (N_L)*[losses[0].iloc[0].values], color="red", label="full network")
    bp.axes[0,1].set_title("Loss")
    bp.axes[0,1].set_ylabel("loss")
    #plt.legend()
    #f.legend()
    ...
    rp.axes[0,0].plot(np.linspace(1, N_L-1, num=N_L), (N_L)*[errors[0].iloc[0].values], color="red")
    rp.axes[0,1].plot(np.linspace(1, N_L-1, num=N_L), (N_L)*[losses[0].iloc[0].values], color="red", label="mean full network")
    #rp.axes[0,1].legend()
    #plt.legend()
    rp.fig.legend()
    ...
    lp.axes.set_ylabel("absolute relative difference")
    lp.axes.set_title("Loss")
    rp.axes[0,1].set_ylabel("loss")
    plt.savefig(fname=os.path.join(dirname, "rel_loss.pdf"))
    ...
    plt.savefig(fname=os.path.join(dirname, "rel_error.pdf"))
   ...
   # parser.add_argument('--log_mult', type=int, default=1, help='The log2 of the mulltiplicative factor')
    ...
    for directory in args.dirs:
        ...
        for f in lst_file:
            ...
            width = args_chkpt.width
            N_L = args_chkpt.depth
            ...
            C = len(quant.columns)
            level_width = C*[width]
            quant.columns = pd.MultiIndex.from_arrays([level_width, quant.columns.get_level_values(0), quant.columns.get_level_values(1)],
