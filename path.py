...
def my_parameters_to_vector(parameters):
...
def point_B(model, solution, ntry, idx_layer, perm, cperm):
    for idx, l in enumerate(solution.network):
        if isinstance(l, nn.ReLU):
            ...
            # elif idx == len(solution.network) - 1:
...
def connect_two_models(path, model, target, sizes):
    ...
    # copy the outcoming weights of the target to the model
    # model.main[-1].weight[-nout:, -nin:]  =  target.main[-1].weight[:nout, :nin]
    # set the output weights of the original model to 0
    # model.main[-1].weight[:nout, :nin].zero_()
    # path.append(model)
    # model.to(device)
    # loss, err = eval_epoch(model, train_loader)
    # print(f"error: {err}, loss: {loss}")
...
if __name__ == "__main__":
    ...
    fn_log_model = "results/cifar10/210331/var-1/logs.txt"
    archi_model = utils.parse_archi(fn_log_model)
    fn_model =  "results/cifar10/210331/var-1/checkpoint.pth"
    ...
    for mid in [1, 2]:
        ...
        for eid in range(n_layer, -1, -1):
            fn_log_sol = f"results/cifar10/210331/var-{mid}/f-2/logs_entry_{eid}.txt"
            fn_solution = f"results/cifar10/210331/var-{mid}/f-2/checkpoint_entry_{eid}.pth"
            chkpt = torch.load(fn_solution, map_location=torch.device('cpu'))
            ...
            # eval_epoch(solution, train_loader, ntry)
            if eid == 0:
