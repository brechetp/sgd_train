    ...
    if args.checkpoint is not None:  # process the  previous computation
        ...
        for m in [m for lst in lst_models for m in lst]:
            ...
            if hasattr(args_model, 'model') and args_model.model.find('vgg') != -1:
                ...
                DropoutClassifier = models.classifiers.DropoutVGG
                ...
                DropoutClassifier = models.classifiers.DropoutFCN
        ...
        # Logs
            ...
            classifier = DropoutClassifier(model, keep).to(device)
...
#     logs = None
            ...
            while classifier.idx_layer >0:
                losses = []
                l = classifier.idx_layer
                for t in range(1, args.steps+1):
                    ...
                    if t % 20 ==0 or t==args.steps or next_try:
                        quant = quant.sort_index(axis=1)
                ...
                classifier.confirm_sample(min_idx)
